...
export class RoomService {
  ...
  async createRoom(payload: RoomDto, createdBy: string) {
  ...
  async updateRoom(payload: RoomDto, updatedBy: string) {
    if (!payload.id)
    ...
    if (payload.outOfOrders) {
      ...
      if (payload.outOfOrders.length > 0) {
        const outOfOrderData = payload.outOfOrders.map((oo) => ({
          ...
          endDate: new Date(oo.endDate),
          updatedBy,
          createdBy: oo.id ? undefined : updatedBy, // If new record, set createdBy too
        }));
