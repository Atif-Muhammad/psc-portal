...
export class ContentService {
    ...
    // --- Events ---
    async createEvent(data: any, createdBy: string, files: Array<Express.Multer.File>) {
    ...
    async updateEvent(id: number, data: any, updatedBy: string, files: Array<Express.Multer.File>) {
    ...
    async createClubRule(data: any, createdBy: string) {
    ...
    async getClubRules(type?: string) {
        const where = type ? { type } : {};
        return this.prisma.clubRule.findMany({ where, orderBy: { createdAt: 'desc' } });
    }

    async updateClubRule(id: number, data: any, updatedBy: string) {
    ...
    async updateAnnouncement(id: number, data: any, updatedBy: string) {
    ...
    // --- About Us ---
    async upsertAboutUs(data: any, auditorName: string) {
        ...
        if (data.id) {
            return this.prisma.aboutUs.update({
                ...
                data: { clubInfo: data.clubInfo, updatedBy: auditorName }
            })
        }
    ...
    // --- Club History ---
    async createClubHistory(data: any, createdBy: string, file?: Express.Multer.File) {
        let imageUrl = null;
        if (file) {
            const upload = await this.cloudinary.uploadFile(file);
            imageUrl = upload.url;
    ...
    // --- Promotional Ads ---
    async createAd(data: any, createdBy: string, file: Express.Multer.File) {
        let imageUrl = null;
        if (file) {
            const upload = await this.cloudinary.uploadFile(file);
            imageUrl = upload.url;
    ...
    async updateAd(id: number, data: any, updatedBy: string, file?: Express.Multer.File) {
