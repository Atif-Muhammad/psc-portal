...
import { Controller, Get, Post, Put, Delete, Body, Param, UseInterceptors, UploadedFile, UploadedFiles, Patch, Req, UseGuards, Query } from '@nestjs/common';
...
export class ContentController {
    ...
    createEvent(@Body() data: any, @UploadedFiles() files: Array<Express.Multer.File>, @Req() req: any) {
    ...
    updateEvent(@Param('id') id: string, @Body() data: any, @UploadedFiles() files: Array<Express.Multer.File>, @Req() req: any) {
        const adminName = req.user?.name || 'system';
    ...
    updateAnnouncement(@Param('id') id: string, @Body() data: any, @Req() req: any) {
        const adminName = req.user?.name || 'system';
    ...
    createHistory(@Body() data: any, @UploadedFile() file: Express.Multer.File, @Req() req: any) {
        const adminName = req.user?.name || 'system';
    ...
    @Put('history/:id')
    @UseInterceptors(FileInterceptor('image'))
    updateHistory(@Param('id') id: string, @Body() data: any, @UploadedFile() file: Express.Multer.File, @Req() req: any) {
        const adminName = req.user?.name || 'system';
    ...
    @Post('ads')
    @UseInterceptors(FileInterceptor('image'))
    createAd(@Body() data: any, @UploadedFile() file: Express.Multer.File, @Req() req: any) {
        const adminName = req.user?.name || 'system';
    ...
    @Put('ads/:id')
    @UseInterceptors(FileInterceptor('image'))
    updateAd(@Param('id') id: string, @Body() data: any, @UploadedFile() file: Express.Multer.File, @Req() req: any) {
        const adminName = req.user?.name || 'system';
