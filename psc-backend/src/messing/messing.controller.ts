import { Body, Controller, Delete, Get, Param, Patch, Post, Request, UseGuards, ParseIntPipe, UseInterceptors, UploadedFiles } from '@nestjs/common';
...
export class MessingController {
    ...
    createCategory(@Body() body: any, @UploadedFiles() files: Array<Express.Multer.File>, @Request() req) {
    ...
    @Patch('item/:id')
    @UseGuards(JwtAccGuard)
    updateItem(@Param('id', ParseIntPipe) id: number, @Body() body: any, @Request() req) {
        const updatedBy = req.user.username || '';
        if (body.price) body.price = Number(body.price);
